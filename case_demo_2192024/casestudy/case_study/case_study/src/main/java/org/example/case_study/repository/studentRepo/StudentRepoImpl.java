...
public class StudentRepoImpl implements IStudentRepo {
    ...
    private static final String UPDATE_STUDENT =
            "update student\n" +
                    ...
                    "class_id = ?\n" +
                    "where student_id = ? ";
    private static final String SEARCH_BY_NAME = "SELECT s.student_id, s.student_name, s.student_birthday, s.student_gender,\n" +
            "s.student_email, s.student_point, c.class_id, c.class_name FROM student s\n" +
            "INNER JOIN class c ON s.class_id = c.class_id WHERE s.student_name LIKE ?;";
    private static final String CHECK_LOGIN = "select * from admin_account\n" +
            "where user_admin = ? and pass_admin = ?";
    ...
    public List<Student> searchByName(String name) {
        ...
        try {
            ...
            list = toList(resultSet); // gọi hàm toList để lặp danh sách
        } catch (SQLException e) {
